...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_usuario'])) {
    ...
    try {
        ...
        $query_elliminarCita = "DELETE FROM cita WHERE id_donante = :id";
        $stmt_eliminarCita = $conn->prepare($query_elliminarCita);
        $stmt_eliminarCita->bindParam(":id", $idUsuario);
        $stmt_eliminarCita->execute();
        $query_eliminarDonante = "DELETE FROM donante WHERE id_donante = :id";
        $stmt_eliminarDonante = $conn->prepare($query_eliminarDonante);
        $stmt_eliminarDonante->bindParam(':id', $idUsuario);
        if ($stmt_eliminarDonante->execute()) {
            $aviso_eliminacion = "<div class='alert alert-success'>Donante eliminado correctamente.</div>";
            require_once("Log.php");
                $log_usuario = new Logs();
                $evento = "Donante eliminado";
                $log_usuario->crear_log($_SESSION['usuario'],$evento);

                //log BBDD
                $id_usuario =$_SESSION['usuario'];
                $operacion = "Eliminar donante";
                $ip = $_SERVER['REMOTE_ADDR'];
                $observacion = "Se ha eliminado el donante con id: $idUsuario";
                ...
                $statement_log_alta = $conn->prepare($query_log_alta);
            ...
            $aviso_eliminacion = "<div class='alert alert-danger'>Error al eliminar el Donante.</div>";
        }
        ...
        <div class="container-fluid px-4 my-5">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    try {
                        $sql = "SELECT id_donante, nombre, apellido1, apellido2, nhc, dni, fecha_nacimiento FROM donante";
                        ...
                        if ($stm->rowCount() > 0) {
                            ...
                            foreach ($stm->fetchAll() as $row) {
                                ...
                                echo "<tr>
                                        ...
                                        <td>
                                            <form method='POST' action='' style='display:inline;' onsubmit='return confirm(\"¿Estás seguro de que deseas eliminar este usuario ".$row['id_donante']." tambien eliminara sus citas asignadas ?\");'>
                                                <input type='hidden' name='id_usuario' value='".$row['id_donante']."'>
